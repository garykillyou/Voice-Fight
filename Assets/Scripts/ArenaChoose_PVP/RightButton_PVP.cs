...
public class RightButton_PVP : MonoBehaviour
{
	...
	public void OnClick ()
	{
		if (bar.value >= -0.01f && bar.value <= 0.01f) {
			GameObject.Find ("Arena_PVP").GetComponent<Arena_PVP> ().CmdRBClick (0.5f);
		} else if (bar.value >= 0.49f && bar.value <= 0.51f) {
			GameObject.Find ("Arena_PVP").GetComponent<Arena_PVP> ().CmdRBClick (1f);
		}
	...
	private void smoothMove ()
	{
		if (bar.value < final) {
			...
			if (bar.value == 0.5f) {
				...
				movie1.GetComponent<movie_PVP> ().GetComponent<AudioSource> ().Stop ();
				...
				movie3.GetComponent<movie_PVP> ().GetComponent<AudioSource> ().Play ();
			}
