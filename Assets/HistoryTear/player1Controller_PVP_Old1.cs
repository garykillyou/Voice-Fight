...
public class player1Controller_PVP_Old1 : NetworkBehaviour
{
        ...
        if (isLocalPlayer)
        {
            ...
            if (GameFunction_PVP.Instance.IsPlaying && block)
            {
                ...
                {      //**********************************  面相右  ********************************************
                    ...
                    if (Input.GetKey(KeyCode.S) && jump) //  ----------------------------------------------------------  S
                    {
                        ...
                        else if (Input.GetKeyDown(KeyCode.Space))   //  蹲下攻擊
                        {
                            playerAnimator.Play("downhitR");
                            block = false;
                            CmdTellServerP1hit(true); //CmdTellServerP1hit(true); //GameFunction_PVP.Instance.p1hit = true;
                            if (GameFunction_PVP.Instance.pos1.x + AttackDistance > GameFunction_PVP.Instance.pos2.x - 26f)
                            {
                                if (GameFunction_PVP.Instance.pos1.y > GameFunction_PVP.Instance.pos2.y - 10f)  //再次確認y軸  高度
                                    CmdTellServerP2hited(true); //CmdTellServerP2hited(true); //GameFunction_PVP.Instance.p2hited = true;
                            }
                        }
                ...
                {*/
                    ...
                    if (GameFunction_PVP.Instance.p1hited)  //受到攻擊時的判定
                    {
                        ...
                        {
                            if (Input.GetKey(KeyCode.S)) playerAnimator.Play("downhitedR");
                            else if (!jump)
                            {
                ...
                {*/
                    ...
                    if (GameFunction_PVP.Instance.p1hited)  //受到攻擊時的判定
                    {
                        ...
                        {
                            if (Input.GetKey(KeyCode.S)) playerAnimator.Play("downhitedL");
                            else if (!jump)
                            {
                                GGrigidbody2D.velocity = new Vector2(-40f, GGrigidbody2D.velocity.y); //擊飛效果
            ...
            else
            {
                ...
                if (rightface)
                {
                    ...
                    if (animatorInfo.IsName("defendR") || animatorInfo.IsName("downdefendR"))  //  若是確實被攻擊到了   要做到p2攻擊時間結束  無法提早動作
    ...
    [Command]
    void CmdTellServerP2hited(bool H)
    {
